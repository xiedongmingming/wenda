...
def chat_init(history):
    ...
    return history_formatted  # (用户，AI(或助手))
...
def load_model():  # 加载模型
    ...
    if len(s) > 1:
        ...
        device_map = {
            'transformer.word_embeddings': start_device,
            'transformer.final_layernorm': start_device,
    ...
    # 根据设备执行不同的操作
    if device == 'cpu':  # 如果是CPU，不做任何操作

        pass

    elif device == 'cuda':  # 如果是GPU，把模型移动到显卡
    ...
    if precision == 'fp16':  # 如果是fp16，把模型转化为半精度

        model = model.half()

    elif precision == 'fp32':  # 如果是fp32，把模型转化为全精度

        model = model.float()

    elif precision.startswith('fp16i'):  # 如果是fp16i开头，把模型转化为指定的精度 从字符串中提取精度的数字部分

        bits = int(precision[5:])

        model = model.quantize(bits)  # 调用quantize方法，传入精度参数
    ...
    elif precision.startswith('fp32i'):  # 如果是fp32i开头，把模型转化为指定的精度 从字符串中提取精度的数字部分
        #
